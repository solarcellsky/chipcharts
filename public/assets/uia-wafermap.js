(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ? factory(exports, require('d3-selection'))
    : typeof define === 'function' && define.amd
    ? define(['exports', 'd3-selection'], factory)
    : ((global = global || self),
      factory((global.uia = global.uia || {}), global.d3));
  ...
  function shotmap_bind(data, x0, y0) {
    ...
    for (var y = 0; y < data.rows; y++) {
      for (var x = 0; x < data.cols; x++) {
        d3.select('#' + this.id() + '_dies_' + (x0 + x) + '_' + (y0 + y)).datum(
          data.die(y, x),
        );
      }
    }
